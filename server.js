// server.js — TDX Arena Deep Tactics Review Backend (COMPLETE FIXED)
...
function getPublicState() { return { ...state }; }
function broadcast() { io.emit('gameStateUpdate', getPublicState()); }
function findViewerByTxId(txId) { return state.allViewers.find(v => String(v.txId) === String(txId)); }
function getViewerByTxIdSafe(txId) { if (!txId) return null; return state.allViewers.find(v => String(v.txId) === String(txId).trim()) || null; }
function findViewerBySocket(sid) { return state.allViewers.find(v => v.id === sid); }

let masterCardPool = [];

async function loadPickedPlayers() {
  try {
    const res = await fetch(PLAYERS_CSV_URL, { headers: { 'Cache-Control': 'no-cache', 'Pragma': 'no-cache' } });
...
setInterval(async () => { const loaded = await loadPickedPlayers(); if (loaded) broadcast(); }, 60000);
...
async function verifyPayment(txId, name) {
  try {
    ...
    return { valid: data.valid === true, amount: parseInt(data.amount || 0, 10), isVIP: parseInt(data.amount || 0, 10) >= VIP_THRESHOLD, matchName: data.name || name };
  } catch (e) { return { valid: false, amount: 0, isVIP: false, matchName: name }; }
}
...
function playAnimationSequence(socket) {
  ...
  function processStep() {
    if (stepIndex >= totalSteps) { pitch.isAnimating = false; io.emit('deepTacticsState', state.deepTactics); return; }
...
app.post('/api/verify', async (req, res) => {
  ...
  if (activeSockets.has(String(txId).trim())) return res.status(400).json({ valid: false, error: 'DUPLICATE_SESSION', message: 'This TDX-ID is already active on another device.' });
...
io.on('connection', (socket) => {
  ...
  socket.on('joinWaitingRoom', async ({ name, ticketCode }) => {
    ...
    if (activeSockets.has(txId)) {
      ...
      if (activeSid !== socket.id) { socket.emit('loginError', { code: 'DUPLICATE_SESSION', message: 'This TDX-ID is active on another device.' }); return; }
    }
    ...
    if (!payment.valid) { socket.emit('loginError', { code: 'INVALID_PAYMENT', message: 'TDX-ID ntabwo yemejwe.' }); return; }
    const viewer = { id: socket.id, txId, name: payment.matchName || String(name).trim(), role: 'spectator', isPremium: payment.isVIP, amount: payment.amount, isVIP: payment.isVIP, secureLink: null };
    state.allViewers.push(viewer);
    activeSockets.set(txId, socket.id);
    sessionStore.set(txId, { name: viewer.name, role: viewer.role, isVIP: viewer.isVIP, amount: viewer.amount, lastSeen: Date.now() });
  ...
  socket.on('refPlacePlayerOnPitch', ({ half, slotIndex, playerId }) => {
    ...
    if (player) {
      ...
      broadcast();
      io.emit('deepTacticsState', state.deepTactics);
      console.log(`✅ Player placed: ${player.name} on ${half} slot ${slotIndex}`);
    }
  });
  ...
  socket.on('refInitDeepTactics', () => {
    ...
    if (!state.deepTactics.pitchState.team1Slots || state.deepTactics.pitchState.team1Slots.length === 0) {
      state.deepTactics.pitchState.team1Slots = new Array(11).fill(null);
    }
    if (!state.deepTactics.pitchState.team2Slots || state.deepTactics.pitchState.team2Slots.length === 0) {
  ...
  socket.on('refAssignReviewFanById', ({ userId, role }) => {
    ...
    if (state.deepTactics.firstReviewFan && state.deepTactics.secondReviewFan) state.deepTactics.phase = 'SETUP';
    broadcast();
    io.emit('deepTacticsState', state.deepTactics);
  });
  ...
  socket.on('refRemoveReviewFan', ({ userId }) => {
    if (!state.deepTactics.active) return;
    if (state.deepTactics.firstReviewFan && String(state.deepTactics.firstReviewFan.id) === userId) state.deepTactics.firstReviewFan = null;
    if (state.deepTactics.secondReviewFan && String(state.deepTactics.secondReviewFan.id) === userId) state.deepTactics.secondReviewFan = null;
  ...
  socket.on('refStartDemonstration', ({ role }) => {
    ...
    if (!fan) { socket.emit('deepTacticsError', { error: `No ${role} review fan assigned` }); return; }
  ...
  socket.on('refStopDemonstration', () => {
    if (!state.deepTactics.active) return;
    const old = state.deepTactics.activeDemonstrator;
    state.deepTactics.activeDemonstrator = null;
    state.deepTactics.phase = 'DEMONSTRATION_READY';
    state.deepTactics.pitchState.selectedSpots = [];
    state.deepTactics.pitchState.animationQueue = [];
    state.deepTactics.pitchState.isAnimating = false;
    if (old) io.to(old.id).emit('demonstrationControl', { hasControl: false });
    broadcast();
    io.emit('deepTacticsState', state.deepTactics);
  });
  ...
  socket.on('refNextReview', () => {
    ...
    if (nextFan && (!current || String(current.txId) !== String(nextFan.txId))) {
      if (current) io.to(current.id).emit('demonstrationControl', { hasControl: false });
  ...
  socket.on('demoMovePlayer', ({ half, slotIndex, newX, newY }) => {
    ...
    const halfSlots = half === 'team1' ? state.deepTactics.pitchState.team1Slots : state.deepTactics.pitchState.team2Slots;
    if (slotIndex < 0 || slotIndex >= 11 || !halfSlots[slotIndex] || !halfSlots[slotIndex].name) return;
    ...
    switch(action) {
      case 'SET_BALL':
        if (spotIndex !== undefined && half) {
          ...
          if (spotIndex >= 0 && spotIndex < 11 && halfSlots[spotIndex] && halfSlots[spotIndex].name) {
      ...
      case 'ADD_ANIMATION_STEP':
        if (direction && spotIndex !== undefined && half) pitch.animationQueue.push({ direction, spotIndex, half, duration: 1000 });
        break;
      case 'PLAY_ANIMATION':
        if (pitch.animationQueue.length > 0) { pitch.isAnimating = true; io.emit('deepTacticsState', state.deepTactics); playAnimationSequence(socket); }
        break;
  ...
  socket.on('demoSelectSpot', ({ half, slotIndex, action }) => {
    ...
    if (action === 'select') { if (!pitch.selectedSpots.includes(spotKey)) pitch.selectedSpots.push(spotKey); }
    else if (action === 'deselect') { pitch.selectedSpots = pitch.selectedSpots.filter(s => s !== spotKey); }
    else if (action === 'clear') { pitch.selectedSpots = []; }
    io.emit('deepTacticsState', state.deepTactics);
  });
  ...
  socket.on('heartbeat', ({ txId }) => {
    ...
    if (!existsInState) { state.allViewers.push(viewer); broadcast(); }
  });
...
setInterval(() => {
  ...
  for (const [txId, data] of heartbeatRegistry.entries()) {
    if (now - data.lastHeartbeat > HEARTBEAT_TIMEOUT && !activeSockets.has(txId)) staleEntries.push(txId);
  }
...
loadPickedPlayers().then(() => {
  state.deepTactics.pitchState.team1Slots = new Array(11).fill(null);
  state.deepTactics.pitchState.team2Slots = new Array(11).fill(null);
  server.listen(PORT, () => console.log(`🧠 Deep Tactics Review server running on port ${PORT}`));
});
